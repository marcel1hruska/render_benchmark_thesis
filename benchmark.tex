...
The benchmark script browses this structure to find the appropriate\\ \texttt{configuration.json} file. These configurations are specific for each test case scenario and renderer, containing the names of the scenes, their description files and specific parameters that are to be passed to the renderer. The purpose of these configurations is that different scenes might need to be rendered in distinct rendering modes (e.g. spectral) or with various variable definitions.
...
Then, he looks up the scenes prepared for Mitsuba2 that test the GGX reflectance and, as these contain only a straightforward geometry,  duplicates them. He saves them in a folder \texttt{/data/cases/reflectance/ART/} and creates \texttt{configuration.json} file in the same folder that only specifies the names of the resulting images, the filenames of the test scenes and the parameters that are to be passed to the renderer (examples can be seen in different ART scenarios).
...
Frodo is a Mitsuba2 developer who just added support for dispersion. As the dispersion has never been tested before, he simply adds a new folder to the benchmark suite \texttt{/data/cases/dispersion/mitsuba2/} and, as in the previous cases, he creates the \texttt{configuration.json} for it.
...
From now on, the benchmark contains the dispersion tests as well. If he wants to provide the reference images as well, these need to be stored in the \texttt{/data/references} folder.
...
Therefore, he rewrites the template scenes from other renderers to his own scene format. He creates a new folder for each test case scenario in the \texttt{/data/cases/}, puts his scenes inside accordingly and creates the \texttt{configuration.json} for each of them.
