...
Frodo is a mitsuba2 developer who recently changed the sampling strategies of the spectral rendering. However, he is not sure whether he broke some of the functionalities. He sets the benchmark to \texttt{mitsuba2} and provides his latest executable. Then, he runs all test case scenarios and compares the results with the reference images.
 
\subsection{Use case New feature}
\label{sec:frodo}
Frodo is an ART developer who would like to add the GGX microfacet distribution to ART as it is not currently supported in the latest version. He follows these steps:

\begin{enumerate}
	\item Find the code snippets that are attached to the benchmark suite and integrate them to ART.
	\item Duplicate the scenes prepared for Mitsuba2 that test the GGX reflectance and store them in a new \texttt{/data/cases/reflectance/ART/} folder. This step should be straightforward as the objects inside the scenes are fairly basic and commented.
	\item For each affected scene, add a JSON snippet to \texttt{configuration.json} file containing the newly supported renderer's name and its parameters (look at \autoref{fig:config}). From that moment on, the benchmark runs the GGX tests for ART as well.
	\item Set the benchmark suite to \texttt{ART} and run it.
\end{enumerate}

The benchmark automatically detects the new configuration for ART. Either from the reference images or from the difference images, Frodo may determine some irregularities that are caused by his incorrect implementation of the masking function. As soon as he corrects it, he sees that the results are coherent.

\subsection{Use case New scenario}

Frodo is a Mitsuba2 developer who just added support for dispersion. As the dispersion has never been tested before, he simply adds a new folder to the benchmark suite \texttt{/data/cases/dispersion/mitsuba2/} and, as in the previous case, he adds the dispersion test case scenario to the \texttt{configuration.json} file.

From now on, the benchmark contains the dispersion tests as well. If Frodo wants to provide the reference images as well, these need to be stored in the \texttt{/data/references} folder.
...
Frodo has created his own spectral renderer, called TheRing, that supports all the test case scenarios in the benchmark suite and he wants to evaluate the correctness of his implementations. He follows these steps:

\begin{enumerate}
	\item Create a new folder \texttt{thering} for each test case scenario in the \texttt{/data/cases/}
	...
	\item Add TheRing to the array containing renderers at the beginning of the\\ \texttt{configuration.json} file
	\item For each scene, add a JSON snippet to the \texttt{configuration.json} file containing the newly supported renderer's name and its parameters. From that moment on, the benchmark fully supports TheRing renderer.
	\item Set the benchmark suite to \texttt{thering} and run it. The benchmark pipeline as well as the visualization work automatically.
\end{enumerate}
