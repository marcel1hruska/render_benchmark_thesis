...
\def\Abstract{%
Due to constant development in computer graphics, various complex rendering techniques offer different approaches to the same aspects of the image synthesis process. Not only the global illumination algorithms, but also appearance descriptions are key distinguishing factors between the rendering systems. These descriptions might include BRDF models, support for spectral color representation but also integration of advanced phenomena such as fluorescence. Unfortunately, as there are no standardized implementations of these features, their computations might not be completely accurate which may result in their incorrect representation.

The aim of this thesis was to introduce an evaluation suite that would methodically test rendering algorithms based on
their appearance reproduction capabilities. The core of the suite is a set of scenes which test five specific appearance phenomena --- polarization, GGX reflectance, fluorescence, iridescence and the overall spectral accuracy. Each test case scenario contains as few scenes as possible while maximizes the number of covered aspects of the specific feature. For the user's convenience, we wrap the scenes inside an automatic workflow that runs the specified test case scenarios and displays the results along with the reference images and their difference images. The reference images are considered to be the ground truth and we manually verified that they are correct, accordingly to their model definitions.
}
